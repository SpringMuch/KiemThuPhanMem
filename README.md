1.Hoàn thiện và nộp ảnh Can't Unsee.
2.Thực hiện viết Test case: Dự án Phân tích dữ liệu điểm số học sinh
2.1. Mô tả bài toán:
Chương trình hỗ trợ giáo viên phân tích danh sách điểm số của học sinh, bao gồm:
- Đếm số học sinh giỏi (điểm >= 8.0).
- Tính điểm trung bình của các học sinh có điểm hợp lệ (0.0 đến 10.0).
2.2. Các trường hợp kiểm thử (Test Cases):
- Dữ liệu chuẩn: Danh sách điểm hỗn hợp hợp lệ và không hợp lệ.
- Giá trị biên: Điểm đúng bằng 0.0 hoặc 10.0.
- Dữ liệu lỗi: Điểm âm hoặc điểm > 10 (phải bị bỏ qua).
- Danh sách rỗng: Trả về kết quả mặc định là 0.
2.3. Hướng dẫn chạy kiểm thử
-  Mở dự án trong môi trường hỗ trợ Java (VS Code, IntelliJ hoặc Eclipse).
-  Đảm bảo thư viện JUnit 5 đã được thêm vào classpath.
-  Chuột phải vào file StudentAnalyzerTest.java và chọn Run Test.
3. Hoàn thiện bài tập Cypress
4.Hoàn thiện bài tập Kiểm thử hiệu năng với JMeter.
